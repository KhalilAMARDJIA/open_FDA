...
`r if(n_significant_peaks > 0) {{paste0(n_significant_peaks, " month(s) identified: ", peak_list)}} else {{"No statistically significant peaks detected at 95% confidence level."}}`

**Statistically Significant Valleys** (≥2 SD below mean, p < 0.05):

`r if(n_significant_valleys > 0) {{paste0(n_significant_valleys, " month(s) identified: ", valley_list)}} else {{"No statistically significant valleys detected at 95% confidence level."}}`

*Note: Months are considered statistically significant outliers when they deviate by ≥2 standard deviations from the mean (z-score ≥ |2|), corresponding to approximately 95% confidence level.*
:::
